...
public class frame1 extends JFrame {
	...
	public frame1() {
		...
		comboBox.setModel(new DefaultComboBoxModel(new String[] {
				"Intance Scan", "Quick Scan", "Port Scan", "Ping Scan" }));
		...
		jlist.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseClicked(java.awt.event.MouseEvent mouseEvent) {
				if (!jlist.getCellBounds(jlist.getSelectedIndex(),
						jlist.getSelectedIndex()).contains(
						mouseEvent.getPoint())) {
					jlist.removeSelectionInterval(jlist.getSelectedIndex(),
							jlist.getSelectedIndex());
				}
				tabbedPane.setSelectedIndex(2);

				/**
				 * OID - .1.3.6.1.2.1.1.1.0 ==> SysDec -Like
				 * " Cisco IOS Software, C2960 Software (C2960-LANBASE-M), Version 12.2(25)FX"
				 * OID - .1.3.6.1.2.1.1.5.0 ==> SysName ÇAlýþýyor OID -
				 * .1.3.6.1.2.1.1.3.0 ==> Uptime Çalýþýyor OID
				 * -1.3.6.1.2.1.11.15.0 ==> Count of Get, OID -
				 * 1.3.6.1.6.3.1.1.6.1.0 ==> one of the set operation.--Kullanma
				 * kafan karýþmasýn. OID - 1.3.6.1.4.1.9.9.13.1.4.1.2.1004 ==>
				 * FanStatus OID - 1.3.6.1.4.1.9.9.13.1.5.1.2.1003 ==>
				 * PowerSupplyStatus OID - 1.3.6.1.4.1.9.2.1.61 ==> Company Info
				 * OID - 1.3.6.1.2.1.16.19.6.0 ==> flash:C2960vsvs OID -
				 * 1.3.6.1.4.1.9.9.23.1.2.1.1.5.10001.1 ==> Ayrýntý Detay En
				 * üstteki gibi. OID - 1.3.6.1.4.1.9.9.23.1.2.1.1.6.10001.1 ==>
				 * Type OID - 1.3.6.1.4.1.9.9.23.1.2.1.1.7.10001.1 ==>
				 * FastEthernet0/1 OID - 1.3.6.1.4.1.9.9.23.1.2.1.1.8.10001.1
				 * ==> Model OID - 1.3.6.1.4.1.9.2.1.56.0 ==>CPU usage 5 second
				 * avg OID - 1.0.8802.1.1.2.1.5.4795.1.2.7.0 ==> Model of the
				 * Device
				 */
				...
				if (flag_load == false) {
					...
					try {
						// SNMPAgent as = new
						// SNMPAgent("192.168.1.10");1.3.6.1.2.1.47.1.1.1.1.13-1.0.8802.1.1.2.1.5.4795.1.2.7.0
						detailsOid = SNMPManager.main(
								"udp:" + jlist.getSelectedValue() + "/161",
								"1.0.8802.1.1.2.1.5.4795.1.2.7.0");//
						...
						if (!detailsOid.equals("SNMP does not supported")) {
							String hostNameOid = SNMPManager.main("udp:"
									+ jlist.getSelectedValue() + "/161",
									"1.3.6.1.2.1.1.5.0");

							// String asd3 =
							// SNMPManager.main("udp:"+jlist.getSelectedValue()+"/161","1.3.6.1.4.1.9.9.13.1.4.1.2.1004");

							// String asd4 =
							// SNMPManager.main("udp:"+jlist.getSelectedValue()+"/161","1.3.6.1.4.1.9.9.13.1.5.1.2.1003");

							String upTimeOid = SNMPManager.main(
									"udp:" + jlist.getSelectedValue() + "/161",
									"1.3.6.1.2.1.1.3.0");

							String asd6 = SNMPManager.main(
									"udp:" + jlist.getSelectedValue() + "/161",
									"1.3.6.1.4.1.9.9.23.1.2.1.1.8.10001.1");

							// String model =
							// SNMPManager.main("udp:"+jlist.getSelectedValue()+"/161",".1.3.6.1.4.1.9.9.23.1.2.1.1.8.1.4");

							// String firmware =
							// SNMPManager.main("udp:"+jlist.getSelectedValue()+"/161","1.3.6.1.4.1.9.9.23.1.2.1.1.5.10001.1");

							String cpuin5scOid = SNMPManager.main("udp:"
									+ jlist.getSelectedValue() + "/161",
									"1.3.6.1.4.1.9.2.1.56.0"); // cpu'ya
																// cevrildi.

							// String testWalk =
							// SNMPWalk.main(jlist.getSelectedValue().toString());

							try {
								doc.insertString(0, "HostName :", keyWord);
								doc.insertString(doc.getLength(), hostNameOid,
										null);
								doc.insertString(doc.getLength(),
										"\n Details : ", keyWord);
								doc.insertString(doc.getLength(), detailsOid,
										null);
								doc.insertString(doc.getLength(),
										"\n Cpu Usage in 5 second (avg): ",
										keyWord);
								doc.insertString(doc.getLength(), "%"
										+ cpuin5scOid, null);
								doc.insertString(doc.getLength(),
										"\n Up Time :", keyWord);
								doc.insertString(doc.getLength(), upTimeOid,
										null);
								doc.insertString(doc.getLength(), "\n Flash :",
										keyWord);
								// doc.insertString(doc.getLength(),
							...
							InetAddress ip1 = InetAddress.getByName(jlist
									.getSelectedValue().toString());

							if (!ip1.isLoopbackAddress()) {
								String hostname = ip1.getHostName();
								String CanonicalHostName = ip1
										.getCanonicalHostName();
								String nbtstat = f.arpQuery(jlist
										.getSelectedValue().toString());
								try {
									doc.insertString(0, "\nNAme :", keyWord);
									...
									doc.insertString(doc.getLength(), nbtstat,
											null);
					...
					for (int k = 0; k < iplist.getItemCount(); k++) {
						textArea_2.setText("");
						if (iplist.getItem(k).equalsIgnoreCase(
							...
							try {
								doc.insertString(0, "HostName :", keyWord);
								doc.insertString(doc.getLength(),
										iplist.getItem(k + 1), null);
								doc.insertString(doc.getLength(),
										"\n Details : ", keyWord);
								doc.insertString(doc.getLength(),
										iplist.getItem(k + 2), null);
								doc.insertString(doc.getLength(), "\n Uptime ",
										keyWord);
								doc.insertString(doc.getLength(),
										iplist.getItem(k + 3), null);
								doc.insertString(doc.getLength(), "\n Flash:",
										keyWord);
								doc.insertString(doc.getLength(),
										iplist.getItem(k + 4), null);
								doc.insertString(doc.getLength(),
				...
				if (flag_scan) {
					...
					for (int i = 1; i < model.getSize(); i++) {
						...
						try {
							...
							asd = SNMPManager.main("udp:" + s + "/161",
									"1.3.6.1.2.1.1.1.0");
							if (!asd.equals("SNMP does not supported")) {
								String asd2 = SNMPManager.main("udp:" + s
										+ "/161", "1.3.6.1.2.1.1.5.0");// Details

								String asd5 = SNMPManager.main("udp:" + s
										+ "/161", "1.3.6.1.2.1.1.3.0");// Uptime

								String asd6 = SNMPManager.main("udp:" + s
										+ "/161", "1.3.6.1.2.1.16.19.6.0");// Flash

								String interfaces = SNMPManager.main("udp:" + s
										+ "/161", "1.3.6.1.4.1.9.2.1.56.0"); // cpu'ya
																				// cevrildi.

								sql.connect(dateFormat.format(date));
								sql.insertData(s, asd, asd2, asd5, asd6,
										interfaces);
					...
					JOptionPane.showMessageDialog(null,
							"Please scan a network", "Save Error",
		...
		btnLoad.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!filename.contains("null")) {
					...
					JOptionPane.showMessageDialog(null,
							"Please select a *.db file", "Loading Error",
							JOptionPane.PLAIN_MESSAGE);
		...
		btnScan.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				int ip_range = f.iki_ip_arasindaki_fark(textField.getText(),
						textField_1.getText());
				...
				if (ip_range >= 1) {
					...
					th = new Thread(new Runnable() {
						public void run() {
							...
							String lastIpCheck = " ";
							while (!(lastIpCheck.equalsIgnoreCase(f
									.nextIpAddress(lastIp)))) {
				...
				// TextFieldCheck'in Else
					...
					// Not sure the grammer for
					// "Start ip is greater then last ip"
					JOptionPane.showMessageDialog(null,
							"Start ip is greater then last ip",
							"Invalid IP Adresses", JOptionPane.PLAIN_MESSAGE);
				}
				// **/
			}
		});
			...
			public boolean shouldYieldFocus(JComponent input) {
				...
				if (inputOK) {
					...
					JOptionPane.showMessageDialog(null,
							...
							"Invalid IP Adress", JOptionPane.PLAIN_MESSAGE);
